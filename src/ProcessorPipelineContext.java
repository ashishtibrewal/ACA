...
public class ProcessorPipelineContext implements IPipelineContext
{
  ...
  private boolean nextInstructionBranchPredictionResult;  /** Branch predictor results for the next instruction - Only useful for branch instructions */
  private boolean currentInstructionBranchPredictionResult; /** Branch predictor results for the current instruction - Only useful for branch instructions */
  ...
  public ProcessorPipelineContext(Register cpuRegisters, Memory cpuMemory, IStage instructionFetchStage, IStage instructionDecodeStage, IStage instructionIssueStage, IStage instructionExecuteStage, IStage instructionWritebackStage, BranchPredictor branchPredictor)
  {
    ...
    this.nextIR = new int[GlobalConstants.BUS_WIDTH];
    this.currentIR = new int[GlobalConstants.BUS_WIDTH];
    Arrays.fill(this.currentIR, GlobalConstants.DEFAULT_INSTRUCTION);         // Default IR value, declared as 0, hence, signifies a NOP operation
    this.nextInstruction = new Instruction[GlobalConstants.BUS_WIDTH];
    this.currentInstruction = new Instruction[GlobalConstants.BUS_WIDTH];
    for (int i = 0; i < GlobalConstants.BUS_WIDTH; i++)
    {
      this.currentInstruction[i] = new Instruction(GlobalConstants.DEFAULT_INSTRUCTION_TYPE,
    ...
    this.nextMemoryFetchLoc = new int[GlobalConstants.BUS_WIDTH];
    this.currentMemoryFetchLoc = new int[GlobalConstants.BUS_WIDTH];
    Arrays.fill(this.currentMemoryFetchLoc,GlobalConstants.DEFAULT_MEM_FETCH_LOC);                                  // Default memory fetch location
  ...
  public void flush()
  {
    Arrays.fill(this.currentIR, GlobalConstants.DEFAULT_INSTRUCTION);  
    for (int i = 0; i < GlobalConstants.BUS_WIDTH; i++)
    {
      this.currentInstruction[i] = new Instruction(GlobalConstants.DEFAULT_INSTRUCTION_TYPE,
                                                   GlobalConstants.DEFAULT_INSTRUCTION_MNEMONIC,

...
public class ProcessorPipelineStatus
{
  ...
  public void print(IPipelineContext context)
  {
    ...
    for (int i = 0; i < GlobalConstants.BUS_WIDTH; i++)
    {
      ...
      int opCode = (instructionFetchStage.getCurrentInstructionRead()[i] >> (Isa.INSTRUCTION_LENGTH - Isa.OPCODE_LENGTH)) & (int)(Math.pow(2, Isa.OPCODE_LENGTH) - 1);     // Extract instruction OpCode (Logical AND with 31 since its 11111 in binary and opcode length is )
      ...
      System.out.println("Bus item number: " + i);
      System.out.println("+-+-------------------------------+-+---------------------------------+-+--------------------------------+-+-----------------------------+-+-----------------------------+-+");
      ...
      System.out.format ("|+| Instruction: 0x%08x       |+| Instruction: 0x%08x         |+| Instruction (Head): 0x%08x |+| Instruction: 0x%08x     |+| Instruction: 0x%08x     |+|%n", instructionFetchStage.getCurrentInstructionRead()[i], instructionDecodeStage.getCurrentInstruction()[i], instructionIssueStage.getCurrentInstruction()[i].getInstructionVal(), instructionExecuteStage.getCurrentInstruction().getInstructionVal(), instructionWritebackStage.getCurrentInstruction().getInstructionVal());
      System.out.format ("|+| Branch Instruction: %5s     |+| Mnemonic   : %4s (%3s)         |+| Mnemonic  : %4s (%3s)         |+| Mnemonic: %4s (%3s)        |+| Mnemonic   : %4s           |+|%n", instructionFetchStage.getBranchInstruction()[i], instructionDecodeStage.getCurrentInstructionMnemonic()[i], instructionDecodeStage.getCurrentInstructionType()[i], instructionIssueStage.getCurrentInstruction()[i].getInstructionMnemonic(), instructionIssueStage.getCurrentInstruction()[i].getInstructionType(), instructionExecuteStage.getCurrentInstruction().getInstructionMnemonic(), instructionExecuteStage.getCurrentInstruction().getInstructionType(), instructionWritebackStage.getCurrentInstruction().getInstructionMnemonic());
      System.out.format ("|+| Branch predictor  : %9s |+| SR1        : R%02d                |+| SR1 (Val) : R%02d (%08d)     |+| SR1 (Val) : R%02d (%08d)  |+| DR         : R%02d            |+|%n", bpResult, instructionDecodeStage.getCurrentSourceReg1()[i], instructionIssueStage.getCurrentInstruction()[i].getSourceReg1Loc(), instructionIssueStage.getCurrentInstruction()[i].getSourceReg1Val(), instructionExecuteStage.getCurrentInstruction().getSourceReg1Loc(), instructionExecuteStage.getCurrentInstruction().getSourceReg1Val(), instructionWritebackStage.getCurrentInstruction().getDestinationRegLoc());
      System.out.format ("|+| PC/Branch target  : %04d      |+| SR2        : R%02d                |+| SR2 (Val) : R%02d (%08d)     |+| SR2 (Val) : R%02d (%08d)  |+| Value      : %08d       |+|%n", cpuRegisters.readPC(), instructionDecodeStage.getCurrentSourceReg2()[i], instructionIssueStage.getCurrentInstruction()[i].getSourceReg2Loc(), instructionIssueStage.getCurrentInstruction()[i].getSourceReg2Val(), instructionExecuteStage.getCurrentInstruction().getSourceReg2Loc(), instructionExecuteStage.getCurrentInstruction().getSourceReg2Val(), instructionWritebackStage.getCurrentInstruction().getWritebackVal());
      System.out.format ("|+|                               |+| DR         : R%02d                |+| DR        : R%02d                |+| Execution Unit: %3s         |+|                             |+|%n", instructionDecodeStage.getCurrentDestinationReg()[i], instructionIssueStage.getCurrentInstruction()[i].getDestinationRegLoc(), instructionExecuteStage.getCurrentInstruction().getExecutionUnit());
      System.out.format ("|+|                               |+| Immediate  : %08d           |+| Immediate : %08d           |+| Prediction Result: %9s|+|                             |+|%n", instructionDecodeStage.getCurrentSignedImmediate()[i], instructionIssueStage.getCurrentInstruction()[i].getSignedImmediateVal(), predictionCorrect);
      System.out.format ("|+|                               |+|                                 |+| Dependency: %5s              |+|                             |+|                             |+|%n", instructionIssueStage.getCurrentInstruction()[i].getDependencyFlag());
      System.out.println("+-+-------------------------------+-+---------------------------------+-+--------------------------------+-+-----------------------------+-+-----------------------------+-+");
    }
    /*
    ...
    */
    System.out.println();
    System.out.println("x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x");
    System.out.println();
